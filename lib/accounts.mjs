...
async function getAccountsInfo(sessionName, url) {
  try {
    const { data } = await axios(url, { timeout: 10 * 1000 });
    ...
    console.log(chalk.yellow("Warning: using accounts info from cache file."));
  }
...
export async function getAccountsWithoutCache({ showSpinner }) {
  ...
  if (!infoCache.length) {
    infoCache = await getAccountsInfo(sessionName, session.vt_accounts_meta_url);
  }
